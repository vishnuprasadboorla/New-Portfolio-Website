  ...
  /workspace/files/git/discard:
    post:
      ...
      x-eov-operation-handler: controllers/GitController
        ...
        -- Set git tracking parameters based on current context
        IF start_parent_id IS NOT NULL AND idx = 1 AND NOT is_repo THEN
            --fetch repo root id using start parent id
            SELECT GIT_REPO_ROOT_ID INTO repo_root_id
            FROM YEEDU.WORKSPACE_FILES
            WHERE ID = start_parent_id;

            effective_is_git       := is_git;
            effective_branch_name  := branch_name;
            effective_lookup_gp_id := lookup_git_providers_id;
            effective_repo_root_id := repo_root_id;
            effective_repo_url     := repo_url;
        ELSIF repo_root_id IS NULL THEN
            effective_is_git       := FALSE;
            effective_branch_name  := NULL;
            effective_lookup_gp_id := NULL;
            effective_repo_root_id := NULL;
